...
import { BrandMark, BrandLabsTag, BRAND_NAME } from '../components/Brand';
...
export default function App() {
  ...
  const [view, setView] = useState<'tracker' | 'panel'>('tracker');
  ...
  const navBtnCls = (active: boolean) =>
    isPanel
      ? `flex items-center gap-2 px-4 py-2 rounded-full transition-colors text-sm font-medium ${
          active ? 'bg-[#E7B24A] text-[#1A1207]' : 'border border-white/15 text-white/70 hover:bg-white/5'
        }`
      : `flex items-center gap-2 px-4 py-2 rounded-full transition-colors text-sm font-medium ${
          active ? 'bg-[#1A1A1A] text-white' : 'border border-black/10 hover:bg-black/5'
        }`;

  return (
    <div
      className={`min-h-screen font-sans transition-colors ${
        isPanel ? 'bg-[#0A0A0B] text-white selection:bg-[#E7B24A] selection:text-black' : 'bg-[#F5F5F0] text-[#1A1A1A] selection:bg-[#5A5A40] selection:text-white'
      }`}
    >
      {/* Header */}
      <header
        className={`border-b backdrop-blur-md sticky top-0 z-10 transition-colors ${
          isPanel ? 'border-white/10 bg-[#0A0A0B]/80' : 'border-black/5 bg-white/50'
        }`}
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                <h1 className={`text-xl font-bold tracking-tight ${isPanel ? 'text-white' : 'text-[#1A1A1A]'}`}>{BRAND_NAME}</h1>
                <BrandLabsTag />
                {isSimulation && (
              ...
              <p className={`text-xs mt-0.5 ${isPanel ? 'text-white/40' : 'text-[#5A5A40] opacity-80'}`}>
                {isPanel ? 'Painel de administração de webhooks' : 'Rastreio em lote e monitoramento via webhook'}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setView('tracker')} className={navBtnCls(view === 'tracker')}>
              <Search className="w-4 h-4" />
              Rastrear
            </button>
            <button onClick={() => { window.location.href = '/panel/webhooks'; }} className={navBtnCls(false)}>
      ...
      <main className={isPanel ? '' : 'max-w-5xl mx-auto px-6 py-12'}>
        
        <div className={`max-w-5xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12 ${view === 'panel' ? 'hidden' : ''}`}>
          {/* Left Column: Input */}
